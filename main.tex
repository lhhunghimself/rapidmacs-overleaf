...
MACS3's peak-calling path is not written in plain Python but compiled to C using Cython, so this is not a comparison between an interpreted implementation and a compiled one; both are compiled. The speed gains come mainly from the data structures and optimizations reported stage by stage in Table~\ref{tab:stages}. Signal tracks are stored as piecewise-constant runs and built by merging already-sorted fragment endpoints in a single sweep, and scale with the number of signal changes rather than with the length of the genome. In the treatment pileup, adjacent runs of equal value are coalesced as they are produced. Where a control is used, the local background is the maximum over four control windows, obtained by merging their already-sorted endpoint streams rather than materializing each window separately. Poisson tail probabilities are memoized per chromosome, because the same (pileup, background) values tend to recur along a track, and the genome-wide $q$-score track is never created. Finally, work is scheduled one chromosome per thread, which is what the 24-thread columns measure.
