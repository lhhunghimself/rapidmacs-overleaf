...
The saving is in how that list is built. The straightforward construction, which our own first implementation used and which is retained as a fallback for unsorted input, emits two events per fragment---$+1$ at its start, $-1$ at its end---sorts them by position, and sweeps the sorted array accumulating a running total. On the ATAC dataset that is 108 million events, and the buffer holding them accounts for roughly 3.8~GB. But the fragments arrive already sorted by start, and their ends form a second sorted stream, so the two can be merged directly: the sort is redundant and the event array never has to exist. The same track comes out of a workspace of a few tens of kilobytes. Adjacent runs of equal value are coalesced as they are produced, so no redundant entry is ever written. Where a control is used, the local background is the pointwise maximum of a genome-wide floor and three control-derived window estimates, obtained by merging the windows' already-sorted endpoint streams rather than materializing each window as its own track. Poisson tail probabilities are memoized per chromosome, because the same (pileup, background) values tend to recur along a track, and the genome-wide $q$-score track is never created. Finally, work is scheduled one chromosome per thread, which is what the 24-thread columns measure.
